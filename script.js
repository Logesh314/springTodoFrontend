...
function login() {
    ...
    .then(response => {
        if(!response.ok){
            throw new Error(data.message || "Registartion failed");
        } 
...
function register() {
    ...
    .then(response => {
        if(response.ok){
            ...
            return response.json().then(data => { throw new Error(data.message || "Registartion failed")});
        }
    }).catch(error => {
        alert(error.message);
    });
...
function loadTodos() {
    ...
    .then(response => {
        if(!response.ok){
            throw new Error(data.message || "Failed get todos")
        } 
        return response.json();
    })
    .then((todos) => {
    ...
    .catch(error => {
        ...
        document.getElementById("todo-list").innerHTML = `<p>failed to loead todos</p>`
    });
}
